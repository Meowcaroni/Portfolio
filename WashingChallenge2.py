...
def run_game_info():
    ...
    def set_difficulty(diff):
        global difficulty_setting
        difficulty_setting = diff

    difficulty_title = Label(text="Difficulty", font=tkFont.Font(family="Times New Roman", size=25), bg=game_background)
    difficulty_title.pack(pady=10)
    difficulty_options = ['easy', 'medium', 'hard']
    clicked = StringVar()
    clicked.set("easy")
    difficulty = OptionMenu(window, clicked, *difficulty_options, command=set_difficulty)
    difficulty.pack()
    gameplay_title = Label(text="Gameplay", font=tkFont.Font(family="Times New Roman", size=25), bg=game_background)
    gameplay_title.pack(pady=10)
